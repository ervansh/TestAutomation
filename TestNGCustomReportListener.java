...
public class TestNGCustomReportListener extends BaseLib implements IReporter {

	private PrintWriter writer;
	private int m_row;
	private Integer m_testIndex;
	private int m_methodIndex;
	//private String reportTitle = "Time Office 9x1 (Standard) - Automation Report";
	private String reportTitle = "SAVIOR TOS 9.2.1[Professional] Rel-9";
	private static final String AUTHOR = "Vanshraj Singh";
	private String reportFileName = "automation-report.html";
	...
	public void generateReport(List<XmlSuite> xmlSuites, List<ISuite> suites, String outdir) {
		...
		startHtml(writer);
		writeReportTitle(reportTitle, AUTHOR);
	...
	private void resultSummary(ISuite suite, IResultMap tests, String testname, String style, String details) {
		...
		if (tests.getAllResults().size() > 0) {
			...
			for (ITestNGMethod method : getMethodSet(tests, suite)) {
				...
				buff.append("<td><a href=\"#m").append(m_methodIndex).append("\">").append(qualifiedName(method)).append(" ");
				...
				//		+ (description != null && description.length() > 0 ? ("" + description + "") : "") + "</a>"
				 ...
				 buff.append("</td>").append("<td class=\"numi style=text-align:left;padding-right:2em\">" + firstLine + "<br/></td>"
								+ "<td style=\"text-align:left\">" + formatter.format(calendar.getTime()) + "</td>"
	...
	/*		buff.append("<td><a href=\"#m").append(m_methodIndex).append("\">")
		        ...
		        buff.append("</td>")
		            ...
		            .append("<td class=\"numi\">").append(timeConversion(end - start)).append("</td>")
		            .append("</tr>");
	...
	*/
	...
	private void startResultSummaryTable(String style) {
		...
		writer.print("<br></br>");
		titleRow("Method Wise Test Execution Summary", 5);
		// writer.print("<br><caption><h3>Test wise Summary
		// Table</h3></caption>");
		writer.println("<tr><th>Class</th>"
				+ "<th>Method</th><th>Exception Information</th><th>Start Time </th><th>Execution Time<br/>(hh:mm:ss)</th></tr>");
		m_row = 0;
	}
	...
	private void resultDetail(IResultMap tests) {
		...
		for (ITestResult result : testResultsList) {
			...
			//writer.println("<h2> id=\"m" + m_methodIndex + "\">" + method.getMethodName() + "</h2>");
	...
	public void generateSuiteSummaryReport(List<ISuite> suites) {
		...
		// add columns in suite.
		tableColumnStart("Browser");
		tableColumnStart("Computer Name");
		tableColumnStart("Domain");
		tableColumnStart("OS");
		...
		for (ISuite suite : suites) {
			...
			for (ISuiteResult r : tests.values()) {
				...
				String hn = env.get("COMPUTERNAME");
				String userdomain = env.get("USERDOMAIN");
				String os = System.getProperty("os.name");
				summaryCell(hn, true);
				summaryCell(userdomain, true);
				summaryCell(os, true);
	...
	protected void writeReportTitle(String title, String author) {
		writer.print("<center><h1>" + title + "</h1></center>");
		writer.print("<center><h3>Date : " + getDateAsString() + " Time :" + getTimeAsString() + "</h3></center>");
		writer.print("<br>");
		// writer.print("<center><h4>Author : "+author+"</h4></center>");
	}
	...
	protected void startHtml(PrintWriter out) {
		...
		out.println("<head>");
		out.println("<title>Automation Report : Vanshraj Singh(QAS)</title>");
	...
	protected void endHtml(PrintWriter out) {
		out.println("<center>Author : <bold>Vanshraj Singh</bold></center>");
